...
-(NSMutableArray *)createTempData : (NSArray *)data{
    ...
    for (int i=0; i<data.count; i++) {
        TreeMacModel *node = [_data objectAtIndex:i];
        if (node.expand) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TreeMapCell *cell = [tableView dequeueReusableCellWithIdentifier:NODE_CELL_ID];
    if (!cell) {
        cell = [[TreeMapCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NODE_CELL_ID];
    }
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    TreeMacModel *node = [_tempData objectAtIndex:indexPath.row];
    
    // cell有缩进的方法
//    cell.indentationLevel = node.depth; // 缩进级别
//    cell.indentationWidth = 15.f; // 每个缩进级别的距离
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    //先修改数据源
    TreeMacModel *parentNode = [_tempData objectAtIndex:indexPath.row];
